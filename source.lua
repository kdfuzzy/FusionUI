...
local requestsDisabled = getgenv and getgenv().DISABLE_Fusion_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.672"
local FusionFolder = "Fusion"
local ConfigurationFolder = FusionFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		FusionOpen = {Type = 'bind', Value = 'K', Name = 'Fusion Keybind'},
		-- buildwarnings
		-- Fusionprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(FusionFolder) then
				if isfile and isfile(FusionFolder..'/settings'..ConfigurationExtension) then
					file = readfile(FusionFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"FusionOpen":{"Value":"K","Type":"bind","Name":"Fusion Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Fusion Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Fusion had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Fusion", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Fusion developers at sirius.menu/discord.")
				end
			end)
...
local Fusion = useStudio and script.Parent:FindFirstChild('Fusion') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Fusion:FindFirstChild('Build') and Fusion.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Fusion | Build Mismatch')
		print('Fusion may encounter issues as you are running an incompatible interface version ('.. ((Fusion:FindFirstChild('Build') and Fusion.Build.Value) or 'No Build') ..').\n\nThis version of Fusion is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Fusion = Fusion, useStudio and script.Parent:FindFirstChild('Fusion') or game:GetObjects("rbxassetid://10804731440")[1]
...
if gethui then
	Fusion.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Fusion)
	Fusion.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Fusion.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Fusion.Name and Interface ~= Fusion then
			Interface.Enabled = false
			Interface.Name = "Fusion-Old"
		end
	end
elseif not useStudio then
...
if Fusion.AbsoluteSize.X < minSize.X and Fusion.AbsoluteSize.Y < minSize.Y then
...
local Main = Fusion.Main
local MPrompt = Fusion:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Fusion:FindFirstChild('Drag')
...
local Notifications = Fusion.Notifications

local SelectedTheme = FusionLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = FusionLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Fusion.Main.BackgroundColor3 = SelectedTheme.Background
	Fusion.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Fusion.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Fusion.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Fusion.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Fusion.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Fusion.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Fusion.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Fusion.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Fusion:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Fusion | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Fusion | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Fusion had an issue decoding the configuration file, please try delete the file and reopen Fusion.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(FusionLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Fusion | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--FusionLibrary:Notify({Title = "Fusion Flags", Content = "Fusion was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(FusionLibrary.Flags) do
...
function FusionLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			FusionLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Fusion'.", Duration = 7, Image = 4400697855})
		else
			FusionLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.FusionOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(FusionFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Fusion Settings', 0, true)

	if TabList['Fusion Settings'] then
		TabList['Fusion Settings'].LayoutOrder = 1000
	end

	if Elements['Fusion Settings'] then
		Elements['Fusion Settings'].LayoutOrder = 1000
	end
...
function FusionLibrary:CreateWindow(Settings)
	if Fusion:FindFirstChild('Loading') then
		if getgenv and not getgenv().FusionCached then
			Fusion.Enabled = true
			Fusion.Loading.Visible = true

			task.wait(1.4)
			Fusion.Loading.Visible = false
		end
	end

	if getgenv then getgenv().FusionCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				FusionLibrary:Notify({Title = 'Build Mismatch', Content = 'Fusion may encounter issues as you are running an incompatible interface version ('.. ((Fusion:FindFirstChild('Build') and Fusion.Build.Value) or 'No Build') ..').\n\nThis version of Fusion is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(FusionFolder) then
		makefolder(FusionFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Fusion"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Fusion Interface Suite" then
		LoadingFrame.Version.Text = "Fusion UI"
	end
	...
	if not Settings.DisableFusionPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(FusionFolder.."/Discord Invites") then
			makefolder(FusionFolder.."/Discord Invites")
		end

		if isfile and not isfile(FusionFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(FusionFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Fusion RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(FusionFolder.."/Key System") then
			makefolder(FusionFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Fusion | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Fusion specific development.')
				end
			end
		end
		...
		if isfile and isfile(FusionFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(FusionFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(FusionFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						FusionLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Fusion | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Fusion specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= FusionLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					FusionLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Fusion.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Fusion | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Fusion specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					FusionLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Fusion | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Fusion specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					FusionLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Fusion | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Fusion specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					FusionLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= FusionLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Fusion | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Fusion specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						FusionLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Fusion | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Fusion specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					FusionLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			FusionLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			FusionLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Fusion had an issue creating settings.') end
...
function FusionLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function FusionLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function FusionLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Fusion Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.FusionOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function FusionLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				FusionLibrary:Notify({Title = "Fusion Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			FusionLibrary:Notify({Title = "Fusion Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Fusion Configurations Error | '..tostring(result))
			FusionLibrary:Notify({Title = "Fusion Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = FusionLibrary:CreateWindow({
	...
	--FusionLibrary:Notify({Title = "Fusion Interface", Content = "Welcome to Fusion. These - are the brand new notification design for Fusion, with custom sizing and Fusion calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(FusionLibrary.Theme) do
...
task.delay(4, function()
	FusionLibrary.LoadConfiguration()
